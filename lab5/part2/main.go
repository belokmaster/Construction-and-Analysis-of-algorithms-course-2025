...
func NewAhoCorasick(wildcard byte) *AhoCorasick {
	root := NewNode(0, nil)
	root.suffixRef = root
	root.parent = root
	return &AhoCorasick{
...
func main() {
	...
	fmt.Scan(&temp)     // Ввод текста
	fmt.Scan(&patt)     // Ввод шаблона
	fmt.Scan(&wildcard) // Ввод символа джокера

	// Инициализация Aho-Corasick с джокером
	ahoCorasick := NewAhoCorasick(wildcard)
